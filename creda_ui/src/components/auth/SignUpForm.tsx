...
// Steps components (same as before)
const PersonalInfoStep = ({ formData, errors, handleChange, handlePasswordChange, handlePasswordVisibility }: any) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
    ...
    <Input id="pan" label="PAN Number" value={formData.pan} onChange={(e:any) => handleChange({ target: { id: 'pan', value: e.target.value.toUpperCase() } })} error={errors.pan} maxLength={10} />
    <Input id="aadhaar" label="Aadhaar Number" value={formData.aadhaar} onChange={handleChange} error={errors.aadhaar} maxLength={14} />
  </div>
);

const EmploymentStep = ({ formData, errors, handleChange }: any) => (
...
const FinancialStep = ({ formData, errors, handleChange }: any) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
...
const DocumentsStep = ({ formData, handleFileChange, errors }: any) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 min-h-[150px]">
...
const SuccessStep = ({ onLogin }: { onLogin?: () => void }) => (
  <div className="text-center py-10 flex flex-col items-center">
    ...
    {/* Optionally allow the user to go straight to dashboard */}
    {onLogin && (
      <div className="mt-6 w-full max-w-xs">
        <button onClick={onLogin} className="w-full px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Go to Dashboard</button>
      </div>
    )}
  </div>
);

export const SignUpForm: React.FC<SignUpFormProps> = ({ setView, onLogin }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev: any) => ({ ...prev, [id]: id === 'aadhaar' ? formatAadhaar(value) : value }));
    if (errors[id]) setErrors((prev: any) => ({ ...prev, [id]: undefined }));
  };

  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = e.target;
    setFormData((prev: any) => ({ ...prev, password: { ...prev.password, value, strength: getPasswordStrength(value) } }));
    if (errors.password) setErrors((prev: any) => ({ ...prev, password: undefined }));
  };

  const handlePasswordVisibility = () => setFormData((prev: any) => ({ ...prev, password: { ...prev.password, visible: !prev.password.visible } }));

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { id, files } = e.target;
    if (files) {
      setFormData((prev: any) => ({ ...prev, [id]: files[0] || null }));
      if (errors[id]) setErrors((prev: any) => ({ ...prev, [id]: undefined }));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // If current step is the Review step (index 4), this submission means signup completed.
    if (currentStep === 4) {
      // call onLogin so App will switch to dashboard
      if (onLogin) onLogin();
      ...
      <div className="bg-gray-900/80 backdrop-blur-xl rounded-xl p-6 md:p-10 shadow-2xl shadow-black/40">
        ...
        <form onSubmit={handleSubmit} className="mt-8">
          <div className={`transition-all duration-300 ${isAnimatingOut ? 'opacity-0' : 'opacity-100'}`}>
            ...
            {currentStep === 5 && <SuccessStep onLogin={onLogin} />}
          </div>

          {currentStep < 5 && (
            <div className="flex justify-between mt-10">
              <button type="button" onClick={handleBack} disabled={currentStep === 0} className="px-6 py-3 rounded-lg font-semibold bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed transition">
                Back
              </button>
